...
> **최종 갱신**: 2026-06-22 (로컬 세션 — **P0~P5 + 코크핏/제출 UI + 콘텐츠 토대 1·2·3·4단계 + 실 데이터 루프 + 중1 전범위(1·2학기) 인제스트 121문항**) · 이전: 토대 1·2단계(실 개념 239개, AI 생성기)
...
**+ 중1 전범위 인제스트 — `lab_problems` 121문항** (합성 30 + 중1-1 49 + **중1-2 42**), 전부 **₩0 세션비전 워크플로**. 문제 검수 브라우저 `/lab/problems`로 품질 확인.
**다음 = 백필 확대**(단원 6 평면도형 각 문제 더 · 다른 교재/학년) 또는 ⑥ 검수큐 · P*b 고도화. 도형 의존 각 문제(평행선·복합)는 Lab 도형 타입 확장 시 인제스트 가능.
...
| DB(공유 Supabase) | `lab_submission_items`(P1)+`lab_submissions.diagnosedAt`(P2). 개념그래프: `lab_concepts` 244행·`lab_concept_edges` 177행. ✅ **토대 3·4단계 스키마**: `lab_problems`에 `body`/`choices`/`explanation`(토대3) + **`diagram`**(토대4) **nullable 컬럼**(db push 비파괴, 행수 불변). 생성/인제스트→영속 글루(`persist.ts`) 완료. **인제스트: `lab_problems` 167행** = 합성시드 30 + **실문제 137**(중1 거의 전범위). 중1 실개념 **26개 중 25개 커버(96%)** — 남은 1개는 14-03 평행선(도형타입 부재로 정상 보류). 소스: 동아(강옥기) 단원마무리(1~8) + 해결해요/수준별학습지 + 단원마무리 5(기본도형). 전부 `isGenerated=false`, **₩0 세션비전 워크플로**(배치-3 OCR→적대적 검증). 도형(삼각형·정다각형·사각뿔·구) 렌더 검증. 난이도 L1~4(L5=0, 단원마무리는 L4까지 — 심화 소스 없음). **워크플로 결과 영속은 `scripts/lab/ingest-workflow-output.ts`**(엔티티 디코딩+diagram 파싱+**객관식 answerIndex↔answer 텍스트 보정**+멱등 source). |
