...
> **최종 갱신**: 2026-06-26 (로컬 세션 — **도형 흑백 렌더(선·글자·각 라벨 스마트 위치) + 지학사 중2 도형 잔여 29문항 검수·적재 → 라이브 DB 검증 총 2,186문항**[중1 1,061·중2 1,125], 도형 109개). 직전: 2026-06-24 중1 소단원학습지 504(총 1,023). ⚠️ 그 후 **중2 전면 인제스트**(동아 318·지학사 258·미래엔 180·YBM 174·천재 90 + 도형 105 = 1,125)가 들어와 1,023→2,186 — 본 갱신에서 실측 반영) · 이전: 2026-06-22 P0~P5 + 코크핏/제출 UI + 콘텐츠 토대 1·2·3·4단계
...
- **총 2,186문항** · 도형(diagram) **109개**. 개념군: mid-13(중1 식·수) 746 · mid-14(중1 기하/통계) 315 · mid-15(중2) 764 · **mid-16(중2 도형) 361**. → **중1 1,061 · 중2 1,125**.
...
**📋 향후 렌더러 보강 과제**(인제스트 가능 도형 확대):
- `segmentLabels` — 중선/대각선 분할 구간 라벨. **위치정보 필드 필요**(`{from,to}` 또는 `{cevianIndex, portion}`) — 현 데이터는 순서·의미 불일치라 자동배치 불가.
- `gridDivisions`/`shadedCells` — 확률 표적(정사각형 N등분 + 색칠). Lab quad 렌더 확장 필요.
- **외심/내심 전용 모드** — 특수점을 변 교점에 + 반지름/접선/이등분선 세그먼트 + 부분각 호. (현 `specialPoints`는 점만 찍음.)
