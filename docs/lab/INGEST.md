...
3-b. **🔴 객관식 answerIndex 0-based/1-based 혼동 → answer 텍스트로 보정** — 실측(기본도형 12문항 중 3건): 에이전트가 1-based 지시에도 **answerIndex=0(무효)·off-by-one**을 내뱉음(answerIndex=3인데 정답이 보기④ 등). 검증자도 *answer 텍스트*만 보고 통과시켜 놓침. **신뢰 신호는 `answer` 텍스트**(에이전트가 계산한 실제 답) — `ingest-workflow-output.ts`가 객관식에서 **answer 텍스트가 보기와 일치하면 그 위치(1-based)로 answerIndex 강제 보정** + 불일치 경고. (스키마에 answerIndex와 answer 텍스트를 *둘 다* 받게 해 교차검증 가능하게 한 게 핵심.) dry-run 로그에서 보정 건수를 보고 스팟체크.
