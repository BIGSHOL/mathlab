...
function buildArticlePrompt(input: ArticleGenerationInput): string {
  ...
  const seedGradeFocus = pick([
    'A등급 전략을 가장 깊게 (상위권이 어디서 무너지는지)',
    'B등급 전략을 가장 깊게 (중상위권의 승부처)',
    'C등급 전략을 가장 깊게 (기초 확보 후 어디까지 올릴 수 있는지)',
  ]);
...
6. **등급별 점수 확보 전략** — **[강조 전략: ${seedGradeFocus}]** A/B/C 등급별 전략을 쓰되, 지정된 등급에 가장 많은 분량을 할당. "~해야 합니다"만 반복하지 말고 현실적 조언
