...
1. **시험 개요** — 핵심 특징 하나로 시작 (예: "서술형 비중이 35점이나 됩니다" 또는 "21문항 중 절반 이상이 응용 이상"). 단순 나열이 아닌, 이 시험의 '성격'을 한 문장으로 규정한 뒤 세부 정보를 풀어서 설명
2. **난이도 분석** — 숫자를 나열하지 말고 "이 숫자가 학생에게 의미하는 바"를 먼저 서술. 끝에 {{CHART:difficulty}} 토큰 삽입
3. **출제 영역 분석** — 출제된 영역의 비중과 특징을 분석. 시험 범위에 포함되지 않아 출제되지 않은 영역(예: 함수, 기하, 확률과 통계)은 언급하지 말 것 — 범위 밖이라 안 나온 건 당연하므로 정보 가치가 없음. 출제된 영역 간의 비중 차이와 학습 전략만 서술. 끝에 {{CHART:type_radar}} 토큰과 {{CHART:ability_radar}} 토큰을 나란히 삽입
4. **단원별 출제 현황** — 단원별 배점의 '이유'를 추측하거나, 학생이 느낄 체감 난이도와 연결. 끝에 {{CHART:topic_bar}} 토큰 삽입
5. **주목할 문항 분석** — 3~5개 문항을 선정하되, 단순 난이도가 아닌 "왜 이 문항이 당락을 가르는가"를 서술. 문항 간에도 연결고리(같은 개념이 여러 문항에 걸쳐 출제된 경우 등)를 짚어줄 것
6. **등급별 점수 확보 전략** — A/B/C 등급별 구체적 전략. "~해야 합니다"만 반복하지 말고, 현실적인 조언 (예: "이 구간에서 2문제만 더 맞히면 등급이 달라집니다")
7. **학습 방향 제안** — 단원 나열이 아닌, 구체적인 학습 순서와 이유를 제시
...
- **이미지 위치**: {{CHART:difficulty}}, {{CHART:type_radar}}, {{CHART:ability_radar}}, {{CHART:topic_bar}} 토큰을 정확히 해당 섹션 끝에 삽입. **절대 "▲ 21문항 난이도 분포" 같은 텍스트 캡션으로 대체하지 말 것!** 반드시 {{CHART:difficulty}} 형태의 토큰 문자열 그대로 출력
- **태그 규칙 (정확히 따를 것)**:
...
- **[중요] JSON 파싱 오류 방지**: 모든 HTML 태그의 속성에는 큰따옴표(") 대신 작은따옴표(')를 사용. ✓ <span style='color: #E03131'> ✗ <span style='color: #E03131'>
...
1. **문장 시작 다양화** — 연속 3문장이 같은 구조로 시작하면 안 됩니다:
   ...
   - 비유/비교로 시작: "시험 범위가 좁다고 방심하면 오히려 독이 됩니다."
