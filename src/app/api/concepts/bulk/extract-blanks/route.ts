...
## hard (2단계 - 심화 암기)
easy에 포함되지 않은 추가 용어. 학생이 내용을 깊이 이해했는지 확인할 수 있는 용어를 최대한 많이 선택.
easy와 통문장(모든 단어 빈칸)의 중간 수준이 되도록, easy 용어 수의 2~3배 이상 선택.
- 성질, 조건, 관계를 나타내는 용어 (예: "평행", "수직", "합동")
- 공식에 나오는 부가 개념 (예: "밑변", "높이")
- 정의/성질을 설명하는 핵심 서술어 (예: "성립한다", "같다", "구한다", "나눈다")
- 수학적 의미가 있는 수식어/형용사 (예: "최대", "최소", "모든", "항상")
- 수식($...$) 중 개념 이해에 필수적인 것
- 숫자가 포함된 핵심 표현 (예: "180도", "2개")
- "~할 수 있다", "~라 한다" 등 의미가 있는 관용구의 핵심 부분
...
7. **절대 금지**: (1), (2), #1, #2 같은 번호 매기기, 순수 숫자("6", "14" 등)를 선택하지 마세요
8. **절대 금지**: □, ▢, ■, ☐ 같은 네모/도형 기호를 용어로 선택하지 마세요. 학생이 입력할 수 없습니다`;
