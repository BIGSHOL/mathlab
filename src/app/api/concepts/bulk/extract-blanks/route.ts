...
## easy (1단계 빈칸) — 고유 용어 6~9개
개념의 뼈대가 되는 핵심 키워드. 한번 읽은 뒤 가볍게 떠올릴 수 있어야 할 것들.
- 정의되는 용어: "~을(를) ~라 한다"에서 정의명
- 공식의 핵심 구성요소: 밑변, 높이, 반지름 등
- 분류/종류명: 정삼각형, 이등변삼각형, 정육면체 등
- **고유 용어 6~9개 (평균 8개 권장). 절대 10개 초과 금지**
- 콘텐츠가 매우 짧아 6개 이상 핵심 용어를 찾기 어렵다면 가능한 만큼 (최소 4개)

## hard (2단계 추가 빈칸) — 고유 용어 6~9개
easy에 포함되지 않은, 수학적으로 의미 있는 나머지 용어.
- 성질/조건/관계 용어: "평행", "수직", "합동", "같다" 등
- 공식의 부가 요소, 단위, 수학 기호가 포함된 표현
- 수식($...$) 중 개념 이해에 필수적인 것
- **고유 용어 6~9개 (평균 8개 권장). 절대 10개 초과 금지. 최소 4개 이상**
- hard가 너무 적으면 2단계 학습이 1단계와 동일해져 의미 없음 — 본문에서 평행/수직/대응/대각/등호 같은 관계 용어, 수식 등을 적극 추출
- 단, 수학과 무관한 일반 서술어는 절대 선택하지 마세요

## 개수 균형 규칙 (중요!)
- easy와 hard 모두 **6~9개 범위, 평균 8개**. 동일해도 무방하나 1~2개 차이를 권장
- 좋은 예: easy 8 + hard 7, easy 7 + hard 8, easy 9 + hard 6
- 피할 예: easy 3 + hard 2 (둘 다 너무 적음), easy 12 + hard 5 (상한 초과)
