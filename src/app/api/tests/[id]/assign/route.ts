...
import { requireTeacher, isResponse, notFound, badRequest, filterAccessibleStudentIds } from '@/lib/api';
...
export async function POST(
  ...
  const validIds = await filterAccessibleStudentIds(currentUser, studentIds);
  const result = await assignTest({ testId, studentIds: validIds, dueDate, allowLateSubmission });
