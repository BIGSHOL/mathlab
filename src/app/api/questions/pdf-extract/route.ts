...
import { collectParams as sharedCollectParams } from '@/lib/utils/diagram-param-collect';
...
⚠️ **절대 최우선 원칙 — 학습 우선(Learning-first extraction)**
이 시스템은 학습 플랫폼입니다. **모든 추출물은 빈칸 학습이 가능한 형태**여야 합니다.
시각적 충실도(교재 원본 그대로의 모양)보다 **학습 가능성이 절대 우선**합니다.

이 원칙에서 파생되는 강제 규칙:
1. **모든 한글 용어/개념명은 반드시 LaTeX($...$) 바깥 마크다운 텍스트로** — "양의 정수", "음의 정수", "소수", "약수" 등 학생이 암기해야 하는 모든 한글 단어
2. **\\text{} 안에 한글 사용 금지** — \\text{양의 정수} 같은 표현은 빈칸 생성기가 감지 못하므로 절대 쓰지 말 것
3. **중괄호 분류도, 표, 분기 다이어그램 등 시각 구조는 마크다운 목록으로 변환** — \\begin{cases}, \\begin{array} 사용 금지 (LaTeX 블록 안에 한글이 갇히게 됨)
4. **LaTeX $...$에는 오직 숫자·변수·기호·수식만** (+1, -2, x, \\frac{1}{2}, \\times, ^{}, \\dots 등)
5. **핵심 한글 용어는 반드시 \`**볼드**\`로 강조** — 빈칸 생성기가 우선 선택함

✅ 올바른 변환 예 (교재: "정수 { 양의 정수 / 0 / 음의 정수" 중괄호 분류도):
\`\`\`
**정수**는 다음과 같이 분류된다:

- **양의 정수**: $+1$, $+2$, $+3$, $\\dots$
- $0$ — 양의 정수도 아니고 음의 정수도 아니다.
- **음의 정수**: $-1$, $-2$, $-3$, $\\dots$
\`\`\`

❌ 금지된 변환 (빈칸 학습 불가):
\`\`\`
$$\\text{정수}\\begin{cases} \\text{양의 정수: } +1, +2, +3, \\dots \\\\ ... \\end{cases}$$
\`\`\`
→ "정수", "양의 정수" 등 핵심 용어가 LaTeX 안에 갇혀 빈칸 추출 불가. **절대 금지**.
...
6. **여러 줄 수식 환경은 반드시 블록 수식($$...$$)으로 감싸기! 인라인 $...$ 안에 절대 넣지 말 것**
   - 대상: \\begin{cases}, \\begin{align}, \\begin{aligned}, \\begin{array}, \\begin{matrix}, \\begin{pmatrix}, \\begin{bmatrix}
   - ❌ 잘못: \`$\\begin{cases} ... \\\\ ... \\end{cases}$\` (인라인 → 파싱 실패, raw 텍스트로 표시됨)
   - ✅ 올바름: \`$$\\begin{cases} ... \\\\ ... \\end{cases}$$\` (블록)
   - 여러 줄 수식은 각 줄이 \\\\로 분리되어야 하며, 이는 인라인에서는 작동하지 않음
7. **분류 다이어그램은 마크다운 목록으로 변환 (LaTeX cases 사용 금지!)**
   - 교재에 "정수 { 양의 정수 / 0 / 음의 정수" 같은 중괄호 분류도가 있어도, 이를 시각적으로 복원하려 하지 마세요
   - 반드시 **볼드 용어 + 불릿 목록** 형태로 변환 (빈칸 학습 가능하도록)
   - 예: 위의 원칙에서 제시한 "정수의 분류" 예시 형태를 따르세요
  ...
  for (const page of pages) {
    try {
      ...
      const fixedProblems = (data.problems || []).map((p: any) => {
        // 공유 유틸 사용
        const collectParams = sharedCollectParams;
