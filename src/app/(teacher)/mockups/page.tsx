...
const ALL_PAGES: PageInfo[] = [
  ...
  { label: '대시보드 v2', href: '/overview/v2', icon: LayoutDashboard, group: 'teacher', description: '새 대시보드 (실험)' },
  ...
  { label: '개념 관리', href: '/concepts', icon: BookOpen, group: 'teacher', description: '수학 개념 CRUD, 빈칸 자동생성, 5단계 학습' },
  ...
  { label: '과정 생성', href: '/courses/create', icon: Route, group: 'teacher', description: '새 학습 과정 만들기' },

  // ─── Teacher: 출제·평가 ───
  { label: '연산 생성기', href: '/questions/arithmetic', icon: Calculator, group: 'teacher', description: '79개 카테고리 연산 문제 생성' },
  { label: 'AI 문제 생성', href: '/questions/generate', icon: Sparkles, group: 'teacher', description: 'AI 기반 문제 자동 생성 (Gemini)' },
  { label: 'PDF 추출', href: '/questions/pdf-import', icon: FileUp, group: 'teacher', description: 'PDF에서 문제 일괄 추출 (Gemini Vision)' },
  ...
  { label: '수기 채점', href: '/manual-grading', icon: CheckSquare, group: 'teacher', description: '서술형 답안 수기 채점' },
  ...
  { label: '학습 분석', href: '/analytics', icon: BarChart3, group: 'teacher', description: '학생별 성취도 분석, 캘린더, 속도 분석' },
  { label: '진단 결과', href: '/diagnostics', icon: AlertTriangle, group: 'teacher', description: '학생별 진단 결과 조회' },
  { label: '리포트', href: '/reports', icon: ScrollText, group: 'teacher', description: '레벨테스트 보고서 관리 (교사용/학부모용)', isNew: true },

  // ─── Teacher: 기타 ───
  { label: '이용권 관리', href: '/licenses', icon: KeyRound, group: 'teacher', description: '학생별 기능 이용권 배정/관리' },
  ...
  { label: '학생 화면 보기', href: '/student-preview', icon: ScanEye, group: 'teacher', description: '학생 시점으로 페이지 확인' },

  // ─── Admin pages ───
  { label: '선생님 관리', href: '/admin/teachers', icon: UserPlus, group: 'admin', description: '[MANAGER+] 소속 선생님 관리' },
  { label: '반 관리', href: '/admin/classrooms', icon: School, group: 'admin', description: '[OWNER+] 반/학급 관리' },
  { label: '사용자 관리', href: '/admin/users', icon: Shield, group: 'admin', description: '[OWNER+] 전체 사용자 관리' },
  { label: '지점 관리', href: '/admin/tenants', icon: Building2, group: 'admin', description: '[SUPER_ADMIN] 전체 지점 관리' },
  { label: '기능 관리', href: '/admin/features', icon: ToggleLeft, group: 'admin', description: '[SUPER_ADMIN] 기능 플래그 관리' },

  // ─── Student pages ───
  { label: '학생 대시보드', href: '/dashboard', icon: GraduationCap, group: 'student', description: '레벨, XP, 숙제, 일일 미션, 복수전' },
  ...
  { label: '프로필', href: '/profile', icon: User, group: 'student', description: '개인 정보, 뱃지, 학습 이력' },

  // ─── Dev pages ───
  { label: '보석 목업', href: '/dev/gems', icon: Gem, group: 'dev', description: '6종 보석 × 5단계 × 4사이즈 SVG 미리보기' },
];
...
export default function MockupsPage() {
  ...
  const groupedPages = useMemo(() => {
    if (activeGroup !== 'all') return { [activeGroup]: filteredPages };
        ...
        return (
          <div key={group} className="mb-8">
            <h2 className="text-base font-bold text-text-primary mb-3 flex items-center gap-2">
              ...
              {cfg.label} 페이지
              <span className="text-xs text-text-secondary font-normal ml-1">({pages.length}개)</span>
            </h2>
