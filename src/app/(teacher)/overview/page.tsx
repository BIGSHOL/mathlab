...
export default async function TeacherDashboard() {
  ...
  return (
    <div className="flex flex-col grow min-w-0 max-w-[1400px] w-full mx-auto p-4 sm:p-6 md:p-10 gap-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <div className="flex gap-3">
          <button className="flex items-center rounded-lg bg-white border border-slate-200 px-3 py-2 text-sm font-semibold text-text-secondary hover:bg-slate-50 transition-all shadow-sm">
            <Calendar className="w-4 h-4 mr-1.5" />
            기간 설정
          </button>
          <button className="flex items-center rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-white hover:bg-primary-hover transition-all shadow-md">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="glass-card flex flex-col gap-2 rounded-2xl p-4 md:p-5 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden"
          >
            <div className="absolute top-0 right-0 p-3 md:p-4">{stat.icon}</div>
            <p className="text-text-secondary text-xs md:text-sm font-semibold tracking-wide">
              {stat.label}
            </p>
            <div className="flex items-end justify-between mt-auto">
              <p className="text-text-primary text-2xl md:text-3xl font-black leading-none">
                {stat.value}
                {stat.suffix && (
                  <span className="text-base md:text-lg text-slate-400 font-bold ml-0.5">{stat.suffix}</span>
                )}
              </p>
            </div>
            <span
              className={`text-xs font-bold px-1.5 py-0.5 rounded flex items-center w-fit ${
                stat.trend.positive
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Chart */}
        <Card variant="glass" className="flex flex-col gap-4 p-5 md:p-6 lg:col-span-3">
          <div className="flex justify-between items-start">
            <div>
              <h3 className="text-text-primary text-lg font-bold">월간 학습 추이</h3>
              <p className="text-text-primary text-2xl font-black mt-1">
        ...
        <Card variant="glass" className="flex flex-col overflow-hidden lg:col-span-2">
          <div className="px-5 py-4 border-b border-slate-200/50 flex justify-between items-center">
              ...
              {focusStudents.length === 0 ? (
                ...
                focusStudents.map((student) => {
                  ...
                  return (
                    <div
                      key={student.id}
                      className="flex items-center gap-3 p-2.5 rounded-xl bg-white/40 hover:bg-white transition-colors shadow-sm"
                    >
                      <div
                      ...
                      <button className="bg-primary/10 hover:bg-primary/20 text-primary p-1.5 rounded-lg transition-colors shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* Grade Distribution */}
        <Card className="p-5">
          <h3 className="text-text-primary text-base font-bold flex items-center gap-2 mb-4">
            <Users className="w-4 h-4 text-primary" /> 학년별 학생 분포
          </h3>
          <div className="flex flex-col gap-3">
            {gradeEntries.length === 0 ? (
              <p className="text-text-secondary text-sm text-center py-4">데이터 없음</p>
            ) : (
        ...
        <Card className="p-5">
          ...
          <div className="flex flex-col gap-2.5">
            {recentActivity.length === 0 ? (
              <p className="text-text-secondary text-sm text-center py-4">아직 학습 기록이 없습니다.</p>
            ) : (
