...
export function EnglishTypeRadarChart({ data, questions }: TypeRadarChartProps) {
  ...
  const standardTypeData = useMemo(() => {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (value <= 0) continue;
      const standard = normalizeQuestionType('ENGLISH', key) || key;
  ...
  const allItems = useMemo(() => {
    if (viewMode === 'ability') {
      return ENGLISH_ABILITY_KEYS.map((key) => ({
        key,
        label: ENGLISH_ABILITY_DOMAIN_LABELS[key],
        value: abilityData[key] || 0,
        color: ENGLISH_ABILITY_DOMAIN_COLORS[key] || '#94A3B8',
      }));
    }
    const keys = (standardTypeData.listening || 0) > 0 ? ENGLISH_QUESTION_TYPE_KEYS : ENGLISH_NAESIN_TYPE_KEYS;
    return keys.map((key) => ({
      key,
      label: ENGLISH_QUESTION_TYPE_LABELS[key],
