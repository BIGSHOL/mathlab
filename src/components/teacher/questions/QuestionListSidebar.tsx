    ...
    <aside className={`shrink-0 border-r border-slate-200 bg-slate-50/30 flex-col transition-all duration-200 ${leftPanelCollapsed ? 'w-12 hidden md:flex' : 'w-full md:w-72'} hidden md:flex`}>
      ...
      {!leftPanelCollapsed && (
        <div className="flex-1 flex flex-col gap-2 p-3 md:p-4 overflow-y-auto">
          ...
          {bookFilter && (() => {
            ...
            <Card padding="sm" className="flex flex-col gap-2">
              <button onClick={() => setSectionOpen((p) => !p)} className="flex items-center justify-between w-full">
                <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider">소단원 필터</h3>
          ...
          {/* 통합 필터 (유형 + 난이도 + 영역 + 출처 + 해설) */}
          <Card padding="sm" className="flex flex-col gap-2">
            <button onClick={() => setFilterOpen((p) => !p)} className="flex items-center justify-between w-full">
              <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider">필터</h3>
              <ChevronDown className={`w-4 h-4 text-text-tertiary transition-transform ${filterOpen ? '' : '-rotate-90'}`} />
            </button>
            {filterOpen && (
              <div className="flex flex-col gap-2.5">
                {/* 유형 (다중 선택 pill) */}
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-text-tertiary shrink-0 w-10">유형</span>
                  <div className="flex flex-wrap gap-1">
                    {TYPE_OPTIONS.map((type) => (
                      <button
                        key={type}
                        onClick={() => { toggleTypeFilter(type); setCurrentPage(1); }}
                ...
                {/* 난이도 */}
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-text-tertiary shrink-0 w-10">난이도</span>
                  <div className="flex flex-wrap gap-1">
                ...
                <div className="flex items-start gap-2">
                  <span className="text-xs font-medium text-text-tertiary shrink-0 w-10 pt-1">영역</span>
                  <div className="flex flex-wrap gap-1">
                    {DOMAIN_OPTIONS.map((d) => (
                      <button
                        key={d.key}
                        onClick={() => { setDomainFilter(d.key); setCurrentPage(1); }}
                        className={`px-2 py-1 rounded-sm text-xs font-medium transition-colors ${
                          domainFilter === d.key
                            ? 'bg-primary text-white'
                            : 'bg-slate-100 hover:bg-slate-200 text-text-secondary'
                ...
                {/* 출처 */}
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-text-tertiary shrink-0 w-10">출처</span>
                  <div className="flex flex-wrap gap-1">
                ...
                <div className="border-t border-slate-100 pt-2 flex flex-col gap-1.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-3.5 h-3.5 rounded-sm border-slate-300 text-primary focus:ring-primary/40"
                    />
                    <span className="text-xs font-medium text-text-secondary">해설 없는 문제만</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={noDiagram}
                      onChange={(e) => { setNoDiagram(e.target.checked); setCurrentPage(1); }}
                      className="w-3.5 h-3.5 rounded-sm border-slate-300 text-primary focus:ring-primary/40"
                    />
                    <span className="text-xs font-medium text-text-secondary">그림 없는 문제만</span>
                  </label>
                </div>
              </div>
            )}
