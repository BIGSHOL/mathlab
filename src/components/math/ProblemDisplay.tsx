                  ...
                  <div className="w-full max-w-lg">
                    {problem.diagramSpec ? (
                      <DiagramRenderer
                        ...
                        className="rounded-sm border border-slate-100 bg-white p-6 shadow-sm"
                      />
                    ) : (
                      <div
    ...
    <div className="h-full flex flex-col text-slate-800">
      ...
      <div className="shrink-0 mb-4">
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-white px-1.5 py-0.5 rounded leading-none bg-primary" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' } as React.CSSProperties}>
                {problem.topic}
              </span>
      ...
      <div className="flex-1">
        ...
        {problem.choices && problem.choices.length > 0 ? (
          <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[12px]">
            {problem.choices.map((choice, idx) => (
              <div key={idx} className="flex items-start gap-2">
                <span className="shrink-0 w-5 h-5 rounded-full border border-slate-300 text-slate-600 text-xs font-medium flex items-center justify-center">
                  {idx + 1}
                </span>
                <div className="flex-1 pt-0.5">
          ...
          <div className="mt-6 h-36 border border-slate-300 rounded-sm p-3">
            <span className="text-slate-400 text-xs">[답안 작성란]</span>
          </div>
        )}
