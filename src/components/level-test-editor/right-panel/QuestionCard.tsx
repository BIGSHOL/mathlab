...
export function QuestionCard({
  ...
  return (
    <div className={`bg-white rounded-sm border border-slate-200 border-l-[3px] ${borderColor} overflow-hidden transition-shadow hover:shadow-md`}>
      <div className="p-4">
        {/* 헤더: 번호 + 단원명 */}
        <div className="flex items-start justify-between mb-2">
          ...
          {showDomain && domain && (
            <span className={`shrink-0 px-2 py-0.5 rounded text-xs font-bold ${domainColor!.bg} ${domainColor!.text}`}>
        ...
        {!questionsOnlyView && (
          <div className="flex items-center gap-1.5 mb-3">
            <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${DIFF_COLORS[question.difficulty] ?? 'bg-slate-100 text-slate-600'}`}>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          <div className="flex items-center gap-1">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-slate-500 hover:text-primary hover:bg-primary/5 rounded transition-colors"
            >
            ...
            <button
              onClick={onRemove}
              className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors"
            >
